# iaf_phase_align.py
# Live Oz IAF + photodiode phase alignment via a simple PLL.
...
PORT = "COM3"
BAUD = 250000
OZ_CH_INDEX = 63      # Oz index in EEG LSL stream
PD_CH_INDEX = 67       # Photodiode index in the SAME LSL stream (change as needed)
BASELINE_SEC = 20.0
CONTROL_HZ = 8        # update @ 8 Hz
BRIGHT_01 = 0.60
...
def main():
    ...
    w.writerow(["t","event","freq_hz","phase_err_rad","meta"])
    def log(evt, f=None, e=None, meta=""): w.writerow([f"{time.monotonic():.6f}", evt, "" if f is None else f"{f:.6f}", "" if e is None else f"{e:.6f}", meta]); logf.flush()
    ...
    try:
        ...
        while True:
            ...
            # Estimate IAF from Oz
            tail_n = int(round(PSD_SEC*fs))
            tail = ozf[-tail_n:] if tail_n<len(ozf) else ozf
            iaf = est_iaf(tail, fs, IAF_SEARCH_LO, IAF_SEARCH_HI)
            if math.isnan(iaf):
                # hold last freq if available
                if last_iaf is not None:
                    play(dev, last_iaf, BRIGHT_01); log("HOLD_IAF", last_iaf, 0.0, "weak_alpha")
                else:
                    dev.cancel_strobe(); log("NO_ALPHA_STOP")
            else:
                # Phase estimates (Oz target vs photodiode)
                # Narrow filter around iaf ± 2 Hz to get clean phase
                lo = max(1.0, iaf-2.0); hi = min(30.0, iaf+2.0)
                oz_n = bandpass(oz[-tail_n:], fs, lo, hi, 2)
                pd_n = bandpass(pd[-tail_n:], fs, lo, hi, 2)
                phi_oz = np.angle(hilbert(oz_n))[-1]
                phi_pd = np.angle(hilbert(pd_n))[-1]
                # wrapped phase error in (-pi,pi]
                err = np.arctan2(np.sin(phi_oz - phi_pd), np.cos(phi_oz - phi_pd))

                # PI control -> frequency correction
                dt = hop
                ...
                log("SET_FREQ_PLL", f_cmd, err, f"iaf={iaf:.2f}")
                last_iaf = f_cmd
